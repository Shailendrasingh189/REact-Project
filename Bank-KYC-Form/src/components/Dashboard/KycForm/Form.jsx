...
import { formSchema, formConfig } from "./validation";
...
const Form = () => {
  const {
    register,
    handleSubmit,
    formState: { errors }, // Corrected here
  } = useForm({
  ...
  return (
    <form
      ...
      noValidate
      className="form min-h-47rem"
    >
      {formConfig.sections.map((section) => (
        <div className="mb-29" key={section.id}>
            ...
            {section.fields.map((field) => {
              ...
              return (
                <InputFields
                  ...
                  register={register}
                  errorMessage={errors[`${name}`]?.message} // Corrected here
                />
              );
            })}
          </div>
